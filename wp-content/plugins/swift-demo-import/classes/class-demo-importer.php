...
if (!class_exists('SDI_Demo_Importer')) {

    final class SDI_Demo_Importer {
        ...
        public static function generate_plugin_label($plugin) {
            ...
            switch ($status) {
                case 'install' :
                    $btn_label = esc_html__('Install', 'swift-demo-import');
                    break;

                case 'inactive' :
                    $btn_label = esc_html__('Activate', 'swift-demo-import');
                    break;

                case 'active' :
                    $btn_label = esc_html__('Installed', 'swift-demo-import');
                    break;
            }
            return $btn_label;
        }
        ...
        public function plugin_installer_callback() {
            ...
            if (!current_user_can('install_plugins')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Sorry, you are not allowed to install plugins on this site.', 'swift-demo-import')
                        )
                );
                die();
            }
            ...
            if (!wp_verify_nonce($nonce, 'plugin_installer_nonce')) {
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => esc_html__('Error - unable to verify nonce, please try again.', 'swift-demo-import')
                        )
                );
                die();
            }
        ...
        public function plugin_offline_installer_callback() {
            ...
            if ($file_location) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Missing File Location.', 'swift-demo-import')
                        )
                );
            }
        ...
        public function plugin_activation_callback() {
            ...
            if (file_exists($plugin_file)) {
                ...
                wp_send_json(
                        array(
                            'success' => false,
                            'message' => __('Error!! Plugin not activated.', 'swift-demo-import')
                        )
                );
            }
        ...
        public function get_local_dir_path($file_location, $plugin) {
            ...
            if (isset($file_location)) {
                ...
                $url = wp_nonce_url(admin_url('themes.php?page=sdi-recommended-plugins'), 'remote-file-installation');
...
SDI_Demo_Importer::get_instance();
